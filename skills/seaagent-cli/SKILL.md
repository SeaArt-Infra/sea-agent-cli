...
Memory commands are scoped by the configured `X-User-ID` production line, `--end-user-id`, and the agent UUID. The gateway derives `tenant_id` and `agent_record_id`; omit scope fields from memory/fact payload files. `agent memory account delete` permanently cascades across worker session storage, medium-term projections, and long-term facts, and requires the CLI's normal interactive delete confirmation. Active facts require a matching `confirmed_by`, normally `user:<end-user-id>`.
...
no-tool smoke test below does not need this approval. It can still invoke
runtime tools if the prompt asks it to; keep smoke-test prompts tool-free.
...
On the current SeaArt gateway, agent `category` is constrained to `fabric`, `seaactor`, or `adk`. Use `fabric` for normal runnable assistants, `seaactor` only for the SeaActor pool, and `adk` only when the Agent is intended for the ADK worker. A known-good model config for SeaArt media agents is:
...
If a newly registered Fabric agent times out even on the no-tool smoke test, update it with the low-level `agent update` shape and set `category: "fabric"` plus the model config above, then retest before debugging tools. Preserve a known `seaactor` or `adk` category while diagnosing its worker pool.

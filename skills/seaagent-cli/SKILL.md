...
seaagent agent memory facts create <agent-id> -f <payload.json|yaml>
seaagent agent memory facts update <agent-id> <fact-id> -f <payload.json|yaml>
seaagent agent memory facts delete <agent-id> <fact-id> [--reason <value>]
```
...
Memory commands are scoped by the configured `X-User-ID` and the agent UUID. The gateway overwrites `user_id` and `agent_record_id`, so omit both fields from memory/fact payload files. Active facts require a matching `confirmed_by`, normally `user:<configured-user-id>`.

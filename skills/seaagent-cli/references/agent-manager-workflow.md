...
- Desired category: `fabric` by default; use `seaactor`, `adk`, or `dsh` only when the corresponding scheduler worker is intentionally required.
...
- `category` must be `fabric`, `seaactor`, `adk`, or `dsh` and must match a deployed scheduler worker pool.

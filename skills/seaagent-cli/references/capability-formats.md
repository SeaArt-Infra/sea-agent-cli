...
`config.max_output_tokens` / `agent_config.max_output_tokens` is an optional positive integer that limits tokens generated by each LLM call. Omit it to preserve the selected model's upstream default. It does not change `max_turns`.

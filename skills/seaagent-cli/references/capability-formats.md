...
- `version` defaults to `v1`; gateway generates `agent_key` from `name:version`; `owner_id` defaults to `internal`.
- Do not send `id` or `agent_key` for new concise agent registrations. Reject or normalize names like `react_game_generator_agent_013919`; use canonical `name: "react_game_generator_agent"` plus an intentional version.
...
Use with `agent register` to create if the payload includes low-level trigger fields, or with `agent update`. Do not include `agent_key`; gateway generates it from `name:v1` for low-level create and from `name:version` for concise register.

...
| MCP servers | `seaagent mcp ...` | Register, list, inspect, update, and delete MCP servers; retain legacy REST diagnostics for tools/call |
...
| `examples/runtime-agent-adk-config.json` | Inline ADK runtime chat config |
...
### Create an Agent from an existing Skill

Reuse active Skills before creating a new Agent. Search, select the returned
Skill UUID, then bind it in `skills`:

```bash
seaagent skill list --search weather --status active --limit 20
seaagent agent register -f payloads/weather-assistant.json
seaagent agent capabilities <agent-id>
seaagent chat run --no-stream <agent-id> "In one sentence, explain what you can do without calling tools."
```

The Agent payload uses the returned UUID, not the Skill name:
...
Use `pre_skills` only when the Skill is needed for most requests. The smoke
test creates a chat run record but does not change registry resources.

Set an Agent's saved default reasoning effort directly:
...
- Use the returned Agent UUID for CLI chat commands. A full chat payload may set `category` to override a registered Agent for one run; omit it for normal runs so the stored category is used.
...
seaagent chat run --agent-config-file examples/runtime-agent-adk-config.json "Run an inline ADK agent"
...
`agent.category`; use the latter in shared config files. Valid values are
`fabric`, `seaactor`, and `adk`. The bundled web and sandbox examples use
`fabric`; `examples/runtime-agent-adk-config.json` is the ADK variant.
...
Streaming writes assistant text to stdout. The CLI prints `run_id`, progress/tool status, terminal usage, and `langfuse_trace_id` to stderr when available. `--no-stream` prints gateway JSON and enriches stored success or failure details, including `response.metadata.langfuse_trace_id`, when chat events are available. Every `chat run` writes a chat run/event record, but it is a smoke-test-safe runtime action rather than a registry mutation.
...
| MCP servers | `register`, `list`, `get`, `update`, `delete`, `tools` (legacy), `call` (legacy) |

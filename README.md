...
`seaagent` is the npm CLI for `agent-gateway` registration, MCP management, discovery, chat, hooks, and sandbox-run workflows.
...
| MCP servers | `seaagent mcp ...` | Register and manage independent MCP servers, discover their tools, and proxy tool calls |
...
4. Registry writes use `user-id` for owner/operator-sensitive gateway behavior; the CLI adds `X-Flag: 1` for Tool, MCP, Skill, and Agent writes.
...
seaagent mcp list --search search --status active
...
Work bottom-up when building a Tool -> Skill -> Agent capability:
...
Manage MCP servers separately from that runtime chain:
...
Tool, MCP server, Skill, and Agent IDs are immutable UUIDs generated by `agent-gateway`. Use returned UUIDs in later `get`, `update`, `delete`, `resolve`, `tools`, `call`, `capabilities`, skill bindings, and chat commands. Hook update and delete use the configured API key as the Hook identity.
...
`pre_skills` entry into the Agent system prompt and avoids that Worker's initial
`read_file` call for `SKILL.md`, at the cost of system-prompt tokens on every
...
it is short. `pre_skills` must be a duplicate-free subset of `skills`. All
bound Skills still keep their required and optional tools.
...
- MCP servers are independent resources. Registering one does not create a Tool record or attach it to Agent runtime.
- `server_url` must be an absolute `http` or `https` URL. `transport` defaults to `streamable-http`; use `sse` only for legacy MCP servers.
- The gateway binds `provider` to the configured `X-User-ID`; do not include it in the payload.
- Gateway responses never return stored upstream header values. They expose `header_keys` only. During an update, omit `headers` to preserve stored values, or send `"headers": {}` to clear them.
- `register`, `update`, and `delete` require a configured `user-id`; the CLI asks for confirmation and sends `X-Flag: 1`. `call` also asks for confirmation because it can execute an arbitrary upstream tool.

## MCP Servers

Manage a registered MCP server independently from the Tool registry:
...
seaagent mcp update <mcp-id> -f payloads/mcp-update.json
seaagent mcp delete <mcp-id>
```

For a private MCP server, `tools` and `call` require the configured `user-id`
to match its provider unless the gateway grants administrative access. The
proxy attaches stored upstream headers, so keep header values in a task-specific
payload and never copy a response into an update file. `call` accepts a tool
`name`, an `arguments` JSON object, and optional `timeout_ms` from `0` through
`120000`; omitted or `0` uses the gateway default of 30 seconds.

Memory notes:

- `agent memory ...` reads one terminal user's memory. Pass `--end-user-id`; the configured `X-User-ID` is the trusted production line.
- `agent memory update/delete` corrects or forgets one medium-term item.
- `agent memory account delete` permanently removes the selected terminal user's worker transcript, semantic memory, and long-term facts. It requires interactive confirmation and is retry-safe when a prior worker purge was interrupted.
...
seaagent agent memory account delete <agent-id> --end-user-id <user-id> --reason account_erasure
seaagent agent memory facts list <agent-id> --end-user-id <user-id> --status active
seaagent agent memory facts create <agent-id> --end-user-id <user-id> -f fact.json
```

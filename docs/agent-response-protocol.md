...
- `category`: scheduling category. Current valid values are `fabric`, `seaactor`, `adk`, and `dsh`. On a registered-Agent chat request, an explicit value overrides the stored category; omit it for normal runs. An inline Agent has no stored category, so it must provide one in the inline config or the request.

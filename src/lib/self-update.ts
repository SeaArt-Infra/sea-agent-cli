...
export async function maybeNotifySkillUpdate(argv: string[]): Promise<void> {
...
export async function getSkillUpdateStatus(): Promise<SkillUpdateStatus> {
...
export async function updateLocalSkill(): Promise<SkillUpdateStatus> {
...
function skillUpdateMessage(status: SkillUpdateStatus): string {
  if (!status.installed) {
...
function isCheckDue(cache: UpdateCheckCache): boolean {
...
async function readCheckCache(): Promise<UpdateCheckCache> {
  try {
    return JSON.parse(await readFile(checkCachePath(), "utf8")) as UpdateCheckCache;
  } catch (err: any) {
    if (err?.code === "ENOENT") {
      return {};
    }
...
async function writeCheckCache(cache: UpdateCheckCache): Promise<void> {
...
function localSkillPath(): string {
...
async function readSkillVersion(path: string): Promise<string> {
  try {

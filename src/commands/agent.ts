...
import { reasoningEffortHelp, updatePayloadWithRuntimeOverrides } from "./agent-update-payload.js";
...
Minimal payload:
  ...
  cmd
    ...
    .option("--max-output-tokens <tokens>", "set the saved maximum tokens generated by each LLM call")
    .addHelpText("after", `

Examples:
  seaagent agent update <agent-id> -f payloads/agent-update.json
  seaagent agent update <agent-id> --reasoning-effort high
  seaagent agent update <agent-id> --max-output-tokens 4096

Use --file for a complete update payload. The runtime options update only their
saved fields, read the current Agent, and preserve its other configuration.`)
    .action(async (agentID: string, options: { file?: string; reasoningEffort?: string; maxOutputTokens?: string }) => {
      if (options.file && (options.reasoningEffort || options.maxOutputTokens)) {
        throw new Error("use either --file or runtime update options, not both");
      }
      if (!options.file && !options.reasoningEffort && !options.maxOutputTokens) {
        throw new Error("--file, --reasoning-effort, or --max-output-tokens is required");
      }
      const client = await AgentGatewayClient.fromConfig();
      const payload = options.file
        ? await readPayload(options.file)
        : updatePayloadWithRuntimeOverrides(await client.get(`/v1/agents/${encodeURIComponent(agentID)}`), {
